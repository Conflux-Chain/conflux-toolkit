...
func MustNewFromBase32(base32Str string) (address Address) {
	...
	if err != nil {
		util.OsExitIfErr(err, "input base32 string: %v", base32Str)
	}
	return
}
...
func MustNewFromHex(hexAddressStr string, networkID ...uint32) (val Address) {
	...
	util.OsExitIfErr(err, "input hex address:%v, networkID:%v", hexAddressStr, networkID)
	return addr
}
...
func MustNewFromCommon(commonAddress common.Address, networkID ...uint32) (address Address) {
	...
	util.OsExitIfErr(err, "input common address:%x, networkID:%v", commonAddress, networkID)
	return addr
}
...
func MustNewFromBytes(hexAddress []byte, networkID ...uint32) (address Address) {
	...
	util.OsExitIfErr(err, "input common address:%x, networkID:%v", hexAddress, networkID)
	return addr
}
...
func (a *Address) GetNetworkID() uint32 {
	...
	util.OsExitIfErr(err, "failed to get networkID of %v", a)
	return id
}
...
func (a *Address) MustGetCommonAddress() common.Address {
	...
	util.OsExitIfErr(err, "failed to get common address of %v", a)
	return addr
}
